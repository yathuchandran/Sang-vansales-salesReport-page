...
import FreeModals from './FreeModalss';
...
const MuiEditableTables = ({ bodyData, outlet, warehouseId, setProductIds, outletid, setBodyData, tableData, newValue, setNewValue, trnsId, }) => {
    ...
    const [freeBatch, setFreeBatch] = useState(null);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [newOpen, setnewOpen] = useState(false);
    const [newOpen1, setnewOpen1] = useState(false); // new modal   
    ...
    const handleAddRow = (rowIndex) => {
        const newRow = { id: generateNewId(), iProduct: '', iUnit: '', fQty: '', Batch: '', fFreeQty: '', iBatch: '', TotalQty: '', fRate: '', fExciseTaxPer: '', Gross: '', fDiscPerc: '', fDiscAmt: '', fAddCharges: '', fVatPer: '', fVAT: '', sRemarks: '', fNet: '', fTotalQty: '', freeBatch: '' };
    ...
    const handleFreeQuantity = (event, rowIndex) => {
        const typedValue = event.target.value;
        const iProduct = formData[rowIndex].iProduct;
        console.log(typedValue,rowIndex);
        ...
        if (Number(typedValue) <= Number(formData[rowIndex].fQty)) {
            ...
            if (Number(typedValue) <= checkValue) {
    ...
    const handlefAddCharges = async (event, rowIndex) => {
        ...
        setFormData(formData.map((row, index) => {
            if (index === rowIndex) {
                return { ...row, fAddCharges: typedValue, fNet: net, fVatPer: vat,discountAmount:calculatedDiscountAmount, };
            }
            return row;
        }));
    ...
    const sumFQty = formData.reduce((accumulator, currentValue) => {
        return accumulator + parseFloat(currentValue.fTotalQty);
    }, 0);
    ...
    const TotdiscountAmount = formData.reduce((accumulator, currentValue) => {
        return accumulator + parseFloat(currentValue.discountAmount);
    }, 0);
    ...
    //handle FREE BATCH BUTTON CLICK-------------------------------------------------------
    const handleFreeButtonClick = (batch, row) => {
        setFreeBatch(batch)
        setFREEFOrmsData(row)
        setnewOpen1(true);
        setMode("new");
    };
                                        ...
                                        <TableCell key={index} sx={{ padding: "0px 0px", border: '1px solid rgba(224, 224, 224, 1)' }} style={{ minWidth: "150px", maxWidth: "300px" }}>
                                            {field.sFieldCaption === "Product" ? (
                                                ...
                                                row.freeBatch && row.freeBatch.length > 0 ? (
                                                    <Button onClick={(event) => handleFreeButtonClick(row.freeBatch, row)}>
                                                        Select Batch
                                                    </Button>
                                                ) : (
                                                    <Typography>NA</Typography>
            ...
            >
                ...
                {showData && (
                    ...
                    >
                        ...
                        <Typography variant="body1" sx={{ marginRight: "16px" }}>
                            Tot Dis Amount: {TotdiscountAmount}
                        </Typography>
                        <Typography variant="body1" sx={{ marginRight: "16px" }}>
...
export default MuiEditableTables;
